...
# Modern Card-Based Design with Deep Shadows
# ==========================================
...
# ==========================================
# 🔍 SMART ML MODEL LOADING - Multi-Format
# ==========================================
...
# Strategy 1: Try XGBoost JSON (Native Format - Recommended)
try:
    import xgboost as xgb
    
    json_path = Path('xgb_model.json')
    config_path = Path('model_config.json')
    
    if json_path.exists() and config_path.exists():
        with open(config_path, 'r', encoding='utf-8') as f:
            model_config = json.load(f)
        
        ml_model = xgb.XGBRegressor()
        ml_model.load_model(str(json_path))
        
        MODEL_STATUS = "xgboost_json"
        model_config['model_type'] = 'xgboost_json'
        load_attempts.append("✅ Loaded from XGBoost JSON")
        
except Exception as e:
    load_attempts.append(f"❌ XGBoost JSON failed: {str(e)}")

# Strategy 2: Try Joblib/Pickle
if MODEL_STATUS != "xgboost_json":
    try:
        import joblib
        
        pkl_path = Path('xgb_model.pkl')
        config_path = Path('model_config.json')
        
        if pkl_path.exists() and config_path.exists():
            ml_model = joblib.load(str(pkl_path))
            with open(config_path, 'r', encoding='utf-8') as f:
                model_config = json.load(f)
            
            MODEL_STATUS = "xgboost_pkl"
            model_config['model_type'] = 'xgboost_pkl'
            load_attempts.append("✅ Loaded from Joblib/Pickle")
            
    except Exception as e:
        load_attempts.append(f"❌ Joblib failed: {str(e)}")

# Strategy 3: Fallback - Simple Model
if MODEL_STATUS not in ["xgboost_json", "xgboost_pkl"]:
    try:
...
<style>
    ...
    * {
    ...
    .main-header {
        ...
        letter-spacing: -1px;
        text-shadow: 0 4px 12px rgba(30, 58, 138, 0.1);
    }
    ...
    /* KPI Cards - Premium with Deep Shadows */
    .kpi-card {
    ...
    .kpi-card:hover {
        ...
        box-shadow: 0 20px 60px rgba(30, 58, 138, 0.15), 0 4px 16px rgba(0, 0, 0, 0.08);
    }
    ...
    /* Property Cards - Premium with Images */
    .property-card {
        background: #ffffff;
        border-radius: 16px;
        overflow: hidden;
    ...
    ::-webkit-scrollbar-thumb:hover { background: linear-gradient(135deg, #3b82f6 0%, #1e3a8a 100%); }
</style>
""", unsafe_allow_html=True)

# ==========================================
# 🔌 DATABASE CONNECTION - بدون LIMIT
# ==========================================
...
if MODEL_STATUS == "xgboost_json":
    ...
    """, unsafe_allow_html=True)
elif MODEL_STATUS == "xgboost_pkl":
    st.markdown(f"""
    <div class="status-banner status-success">
        ✅ <b>XGBoost Model Active (Pickle)</b> | R²: {model_config.get('r2_score', 0.995):.4f} | 
        Accuracy: {100-model_config.get('mape', 3.0):.1f}%
    </div>
    """, unsafe_allow_html=True)
elif MODEL_STATUS == "fallback_linear":
    st.markdown("""
    <div class="status-banner status-warning">
        ⚠️ <b>Fallback Mode</b> - Using Linear Regression. Please upload XGBoost model files.
    </div>
    """, unsafe_allow_html=True)
else:
    st.markdown("""
...
with tab2:
    ...
    if MODEL_STATUS in ["xgboost_json", "xgboost_pkl", "fallback_linear"] and ml_model is not None:
        ...
        if st.button("🎯 Predict Price", type="primary"):
            if MODEL_STATUS in ["xgboost_json", "xgboost_pkl"]:
                # XGBoost prediction
        ...
        st.warning("⚠️ ML Model not available. Please upload model files.")
                ...
                if i + j < len(filtered_df):
                    ...
                    with col:
                        ...
                        if img_url and pd.notna(img_url):
                            ...
                            image_html = '<div class="property-image" style="display: flex; align-items: center; justify-content: center; color: white; font-size: 1rem;">No Image</div>'
...
with tab5:
    ...
    for persona in personas:
        ...
        <div class="persona-card" style="border-left-color: {persona['color']};">
            ...
            <p style="margin: 0; color: #475569; font-size: 1rem; line-height: 1.8;">
